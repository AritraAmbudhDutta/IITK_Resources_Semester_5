...
{	
	...
	// if segment in mms area
	for(int i = 0; i < MAX_MM_SEGS; i++){
		if(i == MM_SEG_STACK && (mms[i].start <= start && end <= mms[i].end - 1) && (mms[i].access_flags & mbit)){
			return 0;
		}
		else if((mms[i].start <= start && end <= mms[i].next_free - 1) && (mms[i].access_flags & mbit)){
			return 0;
		}
	}
	// if segment in vm area
	while(vm_area != NULL){
		if((vm_area->vm_start <= start && end <= vm_area->vm_end - 1) && (vm_area->access_flags & mbit)){
			return 0;
		}
...
long trace_buffer_close(struct file *filep)
{
	...
	if(filep->trace_buffer==NULL || filep->fops == NULL){
		return -EINVAL;
	}

	if(filep->trace_buffer->head_of_trace_buffer == NULL){
		return -EINVAL;
	}
...
int Helper_trace_buffer_read(struct file *filep, char *buff, u32 count)
{
	...
	filep->offp = trace_p->read_offs_of_trace_buffer;
...
int trace_buffer_read(struct file *filep, char *buff, u32 count){
	...
	////////// Performing mem_check //////////
	unsigned long buff_addr = (unsigned long) &buff[0];
	if(is_valid_mem_range(buff_addr, count, 0)<0) return -EBADMEM;

	int ret = Helper_trace_buffer_read(filep,buff,count);
...
int trace_buffer_write(struct file *filep, char *buff, u32 count){
	...
	int ret = Helper_trace_buffer_write(filep,buff,count);

	return ret;
}

int Helper_trace_buffer_write(struct file *filep, char *buff, u32 count)
{	
	...
	filep->offp = trace_p->write_offs_of_trace_buffer;
...
int sys_create_trace_buffer(struct exec_context *current, int mode)
{
	...
	*/
	...
	if(trace_p->head_of_trace_buffer==NULL){
		return -ENOMEM;
	}
...
{	
	...
	int ret = Helper_trace_buffer_write(filep,&syscall_num,8);
	...
	if(n_args==1){
		// code
		ret = Helper_trace_buffer_write(filep,&param1,8);
	...
	if(n_args==2){
		...
		ret = Helper_trace_buffer_write(filep,&param2,8);
	...
	if(n_args==3){
		...
		ret = Helper_trace_buffer_write(filep,&param3,8);
	...
	if(n_args==4){
		...
		ret = Helper_trace_buffer_write(filep,&param4,8);
		// if(ret!=8) printk("OS_PT: Couldn't write meta param4\n");
	}

	u64 meta_data = Delimiter;
	ret = Helper_trace_buffer_write(filep,&meta_data,8);
...
int sys_strace(struct exec_context *current, int syscall_num, int action)
{
	...
	if(action == ADD_STRACE){
		...
		if(base_head->count==STRACE_MAX){
			return -EINVAL;
		}
		//Check if already there
		struct strace_info* check_ptr = base_head->next;
		while(check_ptr){
			if(check_ptr->syscall_num == syscall_num){
				return -EINVAL;
			}
...
int sys_read_strace(struct file *filep, char *buff, u64 count)
{
	...
	while(count>0){
		ret = Helper_trace_buffer_read(filep,&syscall_num,8);
		if(ret<0){
			return -EINVAL;
		}
		...
		ret = Helper_trace_buffer_read(filep,buff+write_offs,n_args*8); //meta not read
		...
		u64 null;
		ret = Helper_trace_buffer_read(filep,&null,8); //meta abhi read kiya
...
int sys_end_strace(struct exec_context *current)
{
	...
	os_free(base_head,sizeof(struct strace_head)); //deallocation
...
void check_and_disable_function(struct exec_context *ctx,struct ftrace_info* fn_info){
	...
	u32* addr32 = fn_info->faddr;	

	if(*addr32 == 0xFFFFFFFF){
		do_ftrace(ctx,fn_info->faddr,DISABLE_FTRACE,0, fn_info->fd);
	}
}
...
long do_ftrace(struct exec_context *ctx, unsigned long faddr, long action, long nargs, int fd_trace_buffer)
{
	...
	if(action == REMOVE_FTRACE){
		...
		struct ftrace_info *ptr = ft_base->next;
		...
		u32 *bit32 = faddr;
		if(*bit32 == 0xFFFFFFFF){
			do_ftrace(ctx, faddr, DISABLE_FTRACE, 0, fd_trace_buffer);
		}
		...
		if(ft_base->next == ptr){
			ft_base->next = ptr->next;
			os_free(ptr, sizeof(struct ftrace_info));
			ft_base->count--;
			return 0;
		}
		struct ftrace_info *prev = ft_base->next;
		while(prev->next != ptr){
			prev = prev->next;
		}
		prev->next = ptr->next;
...
void write_args(struct user_regs* regs,u32 num_args,struct file* filep){
	...
	*/
	...
	ret = Helper_trace_buffer_write(filep,&(regs->rdi),8);
	if(num_args==1) return;

	ret = Helper_trace_buffer_write(filep,&(regs->rsi),8);
	if(num_args==2) return;

	ret = Helper_trace_buffer_write(filep,&(regs->rdx),8);
	if(num_args==3) return;

	ret = Helper_trace_buffer_write(filep,&(regs->rcx),8);
	if(num_args==4) return;

	ret = Helper_trace_buffer_write(filep,&(regs->r8),8);
	if(num_args==5) return;

	ret = Helper_trace_buffer_write(filep,&(regs->r9),8);
	return;
...
long handle_ftrace_fault(struct user_regs *regs)
{
	...
	*((u64 *) regs->entry_rsp) = regs->rbp;
	...
	int ret;
	//writing func addr
	ret = Helper_trace_buffer_write(filep,&faddr,8);
	...
	if(fn_info->capture_backtrace == 1){
		...
		while(ret_addr != END_ADDR){
			...
			ret = Helper_trace_buffer_write(filep,&ret_addr,8);
...
int sys_read_ftrace(struct file *filep, char *buff, u64 count)
{
	...
	while(count>0){
		ret = Helper_trace_buffer_read(filep,&data,8);
		if(ret==0){
