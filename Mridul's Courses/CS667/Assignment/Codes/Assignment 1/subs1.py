...
def refresh_charts():   # update charts
    global data_df

    # Plot temperature data with custom color
    temp_fig = px.line(data_df, x='timestamp', y='temperature', title='Temperature Trend')
    temp_fig.update_traces(line=dict(color='red'))
    temp_chart_area.plotly_chart(temp_fig, use_container_width=True)

    # Plot humidity data with custom color
    hum_fig = px.line(data_df, x='timestamp', y='humidity', title='Humidity Trend')
    hum_fig.update_traces(line=dict(color='blue'))
    ...
    # Plot distance data with filtering and custom color
    distance_filtered_df = data_df[data_df['distance'] < 60]
    distance_fig = px.line(distance_filtered_df, x='timestamp', y='distance', title='Distance Measurement Trend')
    distance_fig.update_traces(line=dict(color='purple'))
    distance_chart_area.plotly_chart(distance_fig, use_container_width=True)

def main():
    mqtt_client_instance = initialize_mqtt_client()     # create instance
    setup_subscription(mqtt_client_instance)
    mqtt_client_instance.loop_forever()     # run loop of client
